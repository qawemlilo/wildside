...
if (!$_POST) {
	...
	if ($_POST["email"] == "") {
		...
		if (mysqli_num_rows($check_res) < 1) {
			...
			//add record
			$add_sql = "INSERT INTO w_people (email) VALUES('".$_POST["email"]."')";
	...
	if ($_POST["email"] == "") {
		...
		if (mysqli_num_rows($check_res) < 1) {
			...
			$del_sql = "UPDATE w_people SET unsubscribe = '1' WHERE people_id = '".$id."'";
...
<html>
<head>
<title>Unsubscribe to the Wildside Magazine Newsletter</title>
</head>
<body>
<h1>Unsubscribe to the Wildside Magazine Newsletter</h1>
